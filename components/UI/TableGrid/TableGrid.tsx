            ...
            positions.map((item, index) => {
              return (
                <React.Fragment key={index}>
                  ...
                  <tr
                    onClick={() => yourPositionOpenHandler(item)}
                    className={`${positions.length == (index + 1) ? 'border-b-[0px]' : 'border-b-[1px]'} ${theme.palette.mode === 'light' ? 'border-black/[0.2]' : 'border-white/[0.1]'} ${styles.rowBottom} cursor-pointer`}
                  >
                    <td className={`${positions.length == (index + 1) ? styles.columnRoundBottomLeft : ''}`}>
                      <span>Strategy Health: 50%</span>
                    </td>
                    <td colSpan={4} className={`${positions.length == (index + 1) ? styles.columnRoundBottomRight : ''}`}>
