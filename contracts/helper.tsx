...
import { ICHI_VAULT_SPELL_ADDR, USDC_ADDR, ICHI_ADDR, BANK_ADDR } from './constants';
...
export const openPosition = async (collateral: string, amount: number, amount1: number) => {
    let { ethereum } = window;
...
export const addCollateral = async (position_id: number, amount: number, leverage: number) => {
    let { ethereum } = window;
    if (typeof window.ethereum !== undefined && window.ethereum) {
        let provider = new ethers.providers.Web3Provider(ethereum);
        let signer = provider.getSigner();
...
export const removeCollateral = async (position_id: number, amount: number, leverage: number) => {
    let { ethereum } = window;
    if (typeof window.ethereum !== undefined && window.ethereum) {
        let provider = new ethers.providers.Web3Provider(ethereum);
        let signer = provider.getSigner();
...
export const depositToken = async (position_id: number, amount: number, leverage: number) => {
    let { ethereum } = window;
    if (typeof window.ethereum !== undefined && window.ethereum) {
        let provider = new ethers.providers.Web3Provider(ethereum);
        let signer = provider.getSigner();
